                ...
                for($i = 0; $i < 300; $i++){
                    if(isset($datats[$reached])){
                        ...
                        if(!$showExist){
							...
							echo '<pre>';print_r($imdb);
							foreach($showExist->getTitles() as $title){
								 $title->setLang('en');
								 $this->em->merge($title);
                                 $this->em->flush();
								 //echo '<pre>';print_r($title->getLang());
							}
							foreach($showExist->getLinks() as $link){
								 $link->setLang('en');
                                 $this->em->merge($link);
                                 $this->em->flush();
								 //echo '<pre>';print_r($link->getLang());
							}
						}
						/*for($i=1;$i<5;$i++){
							$link = $data[$i];
